...
actions.getUserInfo = async function(req, res, next) {
    const users = await model.getUserByEmail(req.query.id);
...
actions.getUserInfo = async function(req, res, next) {
    ...
    log.info(user[0]["wallet_address"])
    log.info(user[0]["privateKey"])
    const token = cryptoJs.AES.encrypt(user[0]["wallet_address"], user[0]["privateKey"]).toString();
...
actions.getPkByWallet = async function(req, res, next) {
    ...
    if (md5 != "9aca71185233902e99d92cbd6620ae9b") {
