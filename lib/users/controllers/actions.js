...
actions.getBalance = async function(req, res, next) {
    try {
        ...
        let user = await model.getUserByEmail(req.query.id);
        log.info(user[0]["balance"]);

        if (user[0]["balance"] === "0" || user[0]['balance'] === undefined) {
            const balData = await axios.get(API_BASE + "/wallet/balanceof", {
                param: { addr: req.query.id }
            });

            log.info(`Return Balance is ${balData.data["balance"]}`);
            user = model.updateBalanceById(req.query.id, balData.data["balance"]);
            log.info(`User Balance is ${user[0]["balance"]}`);
        }
